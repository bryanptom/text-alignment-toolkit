...
def create_homoglyph_dict(sensitivity = 0.35, homoglyph_fp = './utils/homoglyph_list.json'):
...
def interior_lowercase(s):
    ...
    for i in range(start_idx, len(s)):
        if i < len(s) - 1:
            ...
            out += curr.lower() if isnt_cap(right) and isnt_cap(left) else s[i]
        else:
            left, curr = s[i-1], s[i]
...
def visual_spell_checker(
    ...
    spell_checked_words = [quick_fixes(w) for w in spell_checked_words]
...
def textline_evaluation(
    ...
    for gt, pred in pairs:
        ...
        if norm_edit_distance:
            if len(gt) > len(pred):
                edit_count += editdistance.eval(pred, gt) / len(gt)
            else:
                edit_count += editdistance.eval(pred, gt) / len(pred)
        else:
            edit_count += editdistance.eval(pred, gt)
